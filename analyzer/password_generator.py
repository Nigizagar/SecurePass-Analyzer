...
def generate_secure_password(length=14, use_upper=True, use_lower=True, use_digits=True, use_symbols=True):
    ...
    # Proteksi jika tidak ada opsi yang dicentang
    if not charset:
        charset = string.ascii_lowercase + string.digits

    # Pastikan password menyertakan setidaknya satu karakter dari tiap jenis yang dipilih
    password = []
    ...
    # Penuhi sisa panjang password yang diminta pengguna
    remaining_length = length - len(password)
    password += [secrets.choice(charset) for _ in range(remaining_length)]

    # Acak list karakter agar posisi karakter wajib di awal tersamarkan secara aman
